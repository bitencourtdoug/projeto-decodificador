...
function encrypt() {
    ...
    document.getElementById('encoded').innerHTML = '<textarea cols="50" rows="16" id="original__text__decoded">' + encryptedText + '</textarea>' + '<button class="copy" id="copy" onclick="copy()">Copiar</button>'
}

function decrypt() {
    var decryptText = textInformed.value.replace(/enter/g, "e").replace(/imes/g, "i").replace(/ai/g, "a").replace(/ober/g, "o").replace(/ufat/g, "u");
    
    document.getElementById('encoded').innerHTML = '<textarea cols="50" rows="16" id="original__text__decoded">' + decryptText + '</textarea>' + '<button class="copy" id="copy" onclick="copy()">Copiar</button>'
}
